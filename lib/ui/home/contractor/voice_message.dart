...
import 'package:plumbata/utils/contact_noises.dart';
import 'package:plumbata/utils/noises.dart';
import 'package:plumbata/utils/widget.dart';
...
class VoiceMessage extends StatefulWidget {
  VoiceMessage({
    ...
    this.waveForm,
    this.noiseCount = 27,
    this.meBgColor = AppColors.lightAccentColor,
    this.contactBgColor = const Color(0xffffffff),
    this.contactFgColor = AppColors.lightAccentColor,
...
class _VoiceMessageState extends State<VoiceMessage>
  ...
  late final double maxNoiseHeight;
  late final noiseWidth;
  ...
  Container _sizerChild(BuildContext context) => Container(
    ...
    constraints: BoxConstraints(maxWidth: 100.w(context) * .8),
    ...
    child: Padding(
      padding: EdgeInsets.symmetric(horizontal: 4.w(context), vertical: 2.8.w(context)),
      child: Row(
  ...
  Widget _playButton(BuildContext context) => InkWell(
    child: Container(
      ...
      width: 10.w(context, widthRatio: widget.widthRatio),
      height: 10.w(context, widthRatio: widget.widthRatio),
      child: InkWell(
        onTap: () =>
        ...
        child: !_audioConfigurationDone
          ...
          size: 5.w(context, widthRatio: widget.widthRatio),
        ),
      ),
    ),
  );
  ...
  void _setDuration() async {
    if (widget.duration != null) {
      ...
      if(widget.audioSrc != null) {
        ...
        var file = await widget.audioFile;
        _audioDuration = await jsAudio.AudioPlayer().setFilePath((file?.path)!);
      }
    }
